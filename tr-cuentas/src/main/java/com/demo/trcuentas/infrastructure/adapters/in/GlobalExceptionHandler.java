...
import com.demo.trcuentas.domain.shared.exceptions.ResourceNotFoundException;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponseError> handleIllegalArgument(IllegalArgumentException ex) {
        log.error("FATAL ERROR (500 Internal Server Error): Error no controlado. Mensaje: {}", ex.getMessage(), ex);
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(new ApiResponseError().status(false).message("Parámetros Inválidos: " + ex.getMessage()));
    }
